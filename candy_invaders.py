...
def spawn_candy():
    ...
    if probability <= SPAWN_PROB:
        row = 0
        col = random.randrange(0, NUM_COLS - 1)
        ...
        if shape == "circle":
            ...
            points = 3
        candy = (row, col, color, shape, points)
...
def move_candies_down():
    ...
    """
    global candies
    candies = [(r + 1, c, color, shape, points) for r, c, color, shape, points in candies]
...
def eat_and_filter_candies():
    ...
    """
    global candies, player_score, computer_score, NUM_ROWS

    
    eaten_candies = [(r, c, color, shape, points) for r, c, color, shape, points in candies if
                     r == MOUTH_ROW and c == mouth_col]
    off_screen = [(r, c, color, shape, points) for r, c, color, shape, points in candies if r >= NUM_ROWS]

    # update player_score (sum of points eaten)
    for r, c, color, shape, points in eaten_candies:
        player_score += points

    # update computer_score (+1 per escaped candy)
    for r, c, color, shape, points in off_screen:
        computer_score += 1

    # rebuild candies (remove eaten + off-screen)
    candies = [(r, c, color, shape, points) for r, c, color, shape, points in candies if
               not (r == MOUTH_ROW and c == mouth_col) and r < NUM_ROWS]
...
def draw_candy_shape(canvas, x, y, color, shape):
    ...
    """
    if shape == "circle":
        canvas.draw_circle((x, y), 8, 2, color, color)
    elif shape == "square":
        canvas.draw_polygon([(x + 8, y + 8), (x + 8, y - 8), (x - 8, y - 8), (x - 8, y + 8)], 2, color, color)
    else:
        canvas.draw_polygon([(x, y + 4), (x + 8, y - 4), (x - 8, y - 4)], 2, color, color)
...
def draw_candies(canvas):
    ...
    """
    # draw candies
    for row, col, color, shape, points in candies:
...
def draw(canvas):
    ...
    # these functions have to be implemented by you
    draw_grid(canvas)
    draw_candies(canvas)
    ...
    canvas.draw_text(f"Player Score: {player_score}      Computer Score:{computer_score}", (0, 30), 20, "White")
